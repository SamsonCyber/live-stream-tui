...
## Summary
- 153 nodes · 250 edges · 17 communities (14 shown, 3 thin omitted)
...
### Community 3 - "cli.py"
Cohesion: 0.20
Nodes (14): run_app(), main(), CLI entry: live-stream graph | demo | pipe | grok | flood., Live dual-line FPS graph TUI (the Hermes tweet chart). Transparent chrome: host…, run_graph_app(), demo_chunks(), find_grok(), flood_chunks() (+6 more)
...
### Community 5 - "bench.py"
Cohesion: 0.27
Nodes (10): BenchPoint, block_stream(), _fps(), make_block(), measure_parse_paths(), Measure before/after markdown repaint cost as blocks stream in. Before: full-…, Map measured ms to display fps with a soft floor for empty noise. Prefer real…, One markdown block (settles on blank line). (+2 more)
...
### Community 10 - "Grok Build integration"
Cohesion: 0.20
Nodes (9): Agent tool: `stream_graph`, Already wired in `C:\Code\grok-build`, Grok Build integration, How to use in Grok, Install local build (Windows), Modes, Standalone crate (still useful), Tests (+1 more)

## Knowledge Gaps
- **16 isolated node(s):** `live-stream-tui`, `Series`, `graphify`, `Already wired in `C:\Code\grok-build``, `Visibility (only when relevant)` (+11 more)
...
- **Why does `GraphApp` connect `GraphApp` to `cli.py`?**
...
- **What connects `live-stream-tui`, `Series`, `graphify` to the rest of the system?**
